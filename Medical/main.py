...
heart_attacks = df_cleaned[df['Result'] == 'positive']
...
heart_attack_by_age = df_cleaned[df_cleaned['Result_binary'] == 1].groupby('AgeGroup').size()
plt.figure(figsize=(8, 5))
sns.barplot(x=heart_attack_by_age.index, y=heart_attack_by_age.values, palette='Reds')
...
diastolic_by_age = df_cleaned.groupby('AgeGroup')['Diastolic blood pressure'].mean().reset_index()
plt.figure(figsize=(10, 6))
sns.barplot(data=diastolic_by_age, x='AgeGroup', y='Diastolic blood pressure', palette='viridis')
...
sns.barplot(x=heart_attacks_by_gender.index, y=heart_attacks_by_gender.values, palette='Blues')
